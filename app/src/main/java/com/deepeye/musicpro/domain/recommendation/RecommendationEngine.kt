...
constructor(
    ...
    suspend fun buildRecommendations(): RecommendationResult =
        withContext(Dispatchers.IO) {
            val now = System.currentTimeMillis()
            val last30Days = now - 30L * 24 * 3600 * 1000

            // 1. Get user's listen history signals
            val ninetyDays = now - 90L * 24 * 3600 * 1000
            dao.deleteOldListenEvents(ninetyDays)

            val topSongs = dao.getTopSongsSince(last30Days, 10)
            val likedSongs = dao.getRecentlyLikedSongs(10)
            val topArtists = dao.getTopArtistsSince(last30Days, 5)
            val likedArtists = dao.getFavoriteArtistsByLikes(5)
            ...
            // 2. Build seed video IDs for related fetching (prioritize Liked Songs)
            val combinedSongs = (likedSongs + topSongs).distinctBy { it.videoId }
            val seedVideoIds = combinedSongs.map { it.videoId }.take(5)
            
            val combinedArtists = (likedArtists + topArtists).distinctBy { it.artistName }
            val seedArtists = combinedArtists.map { it.artistName }.take(3)
            ...
            val allCandidates =
                (relatedLists + artistLists + contextual)
                    .distinctBy { it.videoId }
                    .filter { it.videoId !in blacklist }

            // 5. Score candidates
            val scored =
                allCandidates.map { video ->
                    val artistBoost =
                        topArtists
            ...
            RecommendationResult(
                // Row 1: Because you played/liked X
                becauseYouListened =
                combinedSongs.take(3).map { song ->
                    val isLiked = likedSongs.any { it.videoId == song.videoId }
                    val titlePrefix = if (isLiked) "Because you Liked" else "Because you played"
                    RecommendationRow(
                        title = "$titlePrefix \"${song.title}\"",
                ...
                favoriteArtists =
                combinedArtists.take(3).map { artist ->
                    val isLiked = likedArtists.any { it.artistName == artist.artistName }
                    val reason = if (isLiked) "Artist you loved" else "${artist.playCount} songs played"
                    RecommendationRow(
                ...
                genreDive =
                topGenres.take(2).map { genre ->
                    RecommendationRow(
                        title = "More ${genre.genre}",
                        subtitle = "Based on your taste",
                        items =
                        scored.filter { it.video.genre.contains(genre.genre, ignoreCase = true) }
