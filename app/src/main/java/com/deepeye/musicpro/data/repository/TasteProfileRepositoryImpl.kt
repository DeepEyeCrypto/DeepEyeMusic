...
@Singleton
class TasteProfileRepositoryImpl
@Inject
constructor(
    ...
    override suspend fun recordFeedback(
        songId: String,
        liked: Boolean,
        dontPlayAgain: Boolean,
    ) {
        val existing = tasteDao.getFeedback(songId)
        val updated =
            existing?.copy(liked = liked, dontPlayAgain = dontPlayAgain)
    ...
    override suspend fun getFeedback(songId: String): UserFeedback? = tasteDao.getFeedback(songId)

    override fun getFeedbackFlow(songId: String): Flow<UserFeedback?> = tasteDao.getFeedbackFlow(songId)

    override suspend fun getNextAutoPlayCandidate(
        ...
        val favoriteLanguagesFromHistory =
            recentEvents
                .groupBy { it.language }
                .mapValues { it.value.size }

        val favoriteArtistsFromHistory =
            recentEvents
                .groupBy { it.artistId }
                .mapValues { it.value.size }
